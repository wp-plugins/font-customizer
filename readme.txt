...
=  Plugin Features =
All settings are done from the WordPress customizer in live preview, which makes this plugin easy as breeze and a really enjoyable experience to use :
...
To improve php performance, all settings, once saved in the customizer, are stored as database options (cached by WordPress when get_option is called) into the database : default settings, custom settings, Google fonts.
